                    ...
                    <div class="card border-0 shadow-sm">
                        <div class="card-body p-2">
                            <form action="{{ route('all-recepit-vochers') }}" method="GET" class="row g-2 align-items-center">
                                <div class="col-md-3">
                                    <h6 class="mb-0 fw-bold text-dark ms-2"><i class="fa fa-money me-2 text-primary"></i>Receipt Vouchers</h6>
                                </div>
                                <div class="col-md-3">
                ...
                <div class="col-12">
                    <div class="card border-0 shadow-sm">
                        <div class="card-header bg-white d-flex justify-content-between align-items-center py-2 border-bottom">
                            <span class="fw-bold text-muted small text-uppercase">Receipts Registry</span>
                        ...
                        <div class="card-body p-0">
                            <div class="table-responsive">
                                <table id="example" class="table table-sm table-striped table-bordered w-100 mb-0">
                                    <thead>
                                        <tr>
                                            <th>ID</th>
                                            <th>Type</th>
                                    ...
                                    <tbody>
                                        ...
                                        <tr>
                                            ...
                                            <td class="text-center small">RV</td>
                                            ...
                                            <td class="text-center">
                                                <span class="badge bg-light text-primary border-0 rounded-pill px-2" style="font-size: 9px;">{{ $item->type_label ?? '-' }}</span>
                                            </td>
                                            <td class="fw-semibold text-dark small">{{ $item->party_name ?? '-' }}</td>
                                            <td class="small text-muted">{{ Str::limit($reference, 20) }}</td>
                                            ...
                                            <td class="text-center">
                                                <div class="d-flex gap-1 justify-content-center">
                                                    @if($item->status === 'draft' || $item->status === 'Unposted')
                                                        ...
                                                        <a href="{{ route('recepit-vochers', $item->id) }}" class="btn btn-outline-warning btn-xs px-1 py-0" title="Edit" style="height: 20px;">
                                                            <i class="fa fa-edit text-dark"></i>
                                                        </a>

                                                        <form action="{{ route('recepit.vochers.cancel', $item->id) }}" method="POST" class="d-inline" onsubmit="return confirm('Delete this unposted voucher?')">
                                                            @csrf @method('DELETE')
                                                    ...
                                                    @else
                                                        <form action="{{ route('recepit.vochers.unpost', $item->id) }}" method="POST" class="d-inline" onsubmit="return confirm('Unpost this voucher?')">
                                                            @csrf
                                                            <button type="submit" class="btn btn-outline-danger btn-xs px-2 py-0" title="Unpost" style="font-size: 10px;">
                                                                <i class="fa fa-undo"></i> Unpost
                                                            </button>
                                                        </form>
                                                    @endif
                                                    
                                                    <a href="{{ route('receiptVoucher.print', $item->id) }}" target="_blank" class="btn btn-outline-dark btn-xs px-1 py-0" title="Print" style="height: 20px;">
...
<script>
    $(document).ready(function() {
        ...
        const storageKey = 'receipt_voucher_table_columns_v2';
