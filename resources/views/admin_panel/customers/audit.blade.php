    ...
    <div class="container-fluid">
        <div class="row mb-3">
            <div class="col-12 d-flex justify-content-between align-items-center">
                <div>
                    ...
                    <p class="text-muted small mb-0">Detailed breakdown of opening, sales, returns, payments, calculated true balance vs system saved balance.</p>
                </div>
                <div>
        ...
        <div class="card border-0 shadow-sm mb-3">
            <div class="card-body p-3">
                <form method="GET" action="{{ route('customers.audit') }}" class="row g-2 align-items-center">
                    <div class="col-md-3">
                    ...
                    <table class="table table-sm table-striped table-bordered align-middle w-100 mb-0" style="font-size: 11px;">
                        ...
                        <tbody>
                            ...
                            <tr class="{{ !$isOk ? 'table-danger' : '' }}">
                                ...
                                <td class="text-end text-success">{{ number_format($r['sales'], 0) }}</td>
                                <td class="text-end">{{ number_format($r['c_rep'], 0) }}</td>
                                <td class="text-end text-danger">{{ number_format($r['s_ret'], 0) }}</td>
                                <td class="text-end text-success">{{ number_format($r['cir'], 0) }}</td>
                                <td class="text-end text-danger">{{ number_format($r['receipts'], 0) }}</td>
                            ...
                            @empty
                            <tr>
                                <td colspan="16" class="text-center p-3 text-muted">No customer record found.</td>
                            </tr>
                            @endforelse
                        </tbody>
                    </table>
