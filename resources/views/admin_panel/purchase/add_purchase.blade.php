    ...
    (function() {
        ...
        @if(isset($purchase))
            @php
                ...
                $pItems = $purchase->items->map(function($item) use ($allProductPrices, $rawDate, $purchaseTime) {
                    $product = $item->product;
                    
                    $retail = 0;
                    $pricesForProd = $allProductPrices->get($item->product_id, collect());
                    ...
                    if ($retail <= 0) {
                        $retail = (float) ($product?->latestPrice?->purchase_retail_price ?? $product?->latestPrice?->sale_retail_price ?? $product?->retail_price ?? 0);
                    }
