...
<div class="main-content">
    <div class="main-content-inner">
        <div class="container-fluid pt-3">
            ...
            <div class="d-flex justify-content-between align-items-center mb-3 bg-light p-2 rounded shadow-sm">
                ...
                <div id="postActionArea" style="min-width:80px;">
                    @if($transfer->status != 'Posted')
                        <button type="button" onclick="$('#postBtn').trigger('click')" class="btn btn-sm btn-primary rounded-pill px-4 shadow-sm">
                ...
                <div class="d-flex align-items-center gap-2 justify-content-center flex-grow-1">
                    ...
                    <span id="statusBadge" class="badge {{ $transfer->status == 'Posted' ? 'bg-success' : 'bg-info text-white' }} px-3 py-2 rounded-pill shadow-sm" style="font-size:12px;">
                        <i class="fa {{ $transfer->status == 'Posted' ? 'fa-check' : 'fa-pencil' }} me-1"></i>
                        {{ $transfer->status }}
                    </span>
            ...
            <form action="{{ route('stock_transfers.update', $transfer->id) }}" method="POST" id="transferForm" class="position-relative {{ $transfer->status == 'Posted' ? 'form-locked' : 'form-locked' }}">
                @csrf
                @method('PUT')
                    ...
                    <div class="card-footer bg-white py-3">
                        <div class="d-flex justify-content-end gap-2">
                            ...
                            <button type="button" id="saveDraftBtn" class="btn btn-sm btn-warning rounded-pill px-4 shadow-sm">
                                <i class="fa fa-floppy-o me-1"></i> Update
                            ...
                            {{-- New --}}
                            <a href="{{ route('stock_transfers.create') }}" id="newInvoiceBtn" class="btn btn-sm btn-info rounded-pill px-4 shadow-sm text-white">
...
$(document).ready(function() {
    ...
    var _isPosted = "{{ $transfer->status == 'Posted' ? 1 : 0 }}";
    ...
    // Populate existing
    var existingItems = @json($transfer->items->map(function($item) {
        return [
            'id' => $item->product_id,
    ...
    function ajaxSaveDraft(callback) {
        ...
        $.ajax({
            url: "{{ route('stock_transfers.update', $transfer->id) }}", type: 'POST', 
            data: $form.serialize(),
            headers: { 'X-Requested-With': 'XMLHttpRequest' },
            success: function(res) {
                if (res.success) {
                    showToast('✅ Updated successfully!');
                    $('#statusBadge').removeClass('bg-warning').addClass('bg-info text-white').html('<i class="fa fa-pencil me-1"></i> Unposted');
                    $('#editInvoiceBtn').show();
            ...
            complete: function() {
                $('#saveDraftBtn').prop('disabled', false).html('<i class="fa fa-floppy-o me-1"></i> Update <kbd style="font-size:9px;opacity:.8;margin-left:4px;">Ctrl+S</kbd>');
            }
        });
    }
        ...
        var html = `
            <div style="border:1px solid #eee;padding:20px;max-width:780px;margin:auto;">
                <div style="display:flex;justify-content:space-between;border-bottom:2px solid #000;padding-bottom:10px;margin-bottom:16px;">
                    ...
                    <div style="text-align:right;font-size:12px;"><div><strong>Transfer #:</strong> ${_savedTransferId}<br><strong>Status:</strong> ${_isPosted == 1 ? 'POSTED' : 'UNPOSTED'}</div></div>
                </div>
                ...
                <div style="display:grid; grid-template-columns:1fr 1fr; gap:16px; margin-bottom:16px; font-size:12px;">
                    <div><div><span style="font-weight:600;width:120px;display:inline-block;">Date:</span>{{ $transfer->created_at->format('d-M-Y') }}</div></div>
                    <div><div><span style="font-weight:600;width:120px;display:inline-block;">Prepared By:</span>{{ $transfer->creator->name ?? '-' }}</div></div>
                </div>
    ...
    if(_isPosted == 1) {
        ...
        // Unposted - Start locked by default, hide buttons until Edit is clicked
        $('#transferForm').addClass('form-locked');
