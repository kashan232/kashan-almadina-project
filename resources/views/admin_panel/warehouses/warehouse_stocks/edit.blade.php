...
<style>
    ...
    .form-locked .remove-row, .form-locked #addRowBtn, .form-locked .product-select + .select2-container { 
        ...
        <div class="container-fluid pt-3">
            ...
            <div class="d-flex justify-content-between align-items-center mb-3 bg-light p-2 rounded shadow-sm">
                ...
                <div class="d-flex align-items-center gap-2 justify-content-center flex-grow-1">
                    <h6 class="page-title mb-0 fw-bold">Edit Stock Adjustment</h6>
                    <span id="statusBadge" class="badge {{ $adjustment->status == 'Posted' ? 'bg-success text-white' : 'bg-info text-white' }} px-3 py-2 rounded-pill shadow-sm" style="font-size:12px;">
                        <i class="fa {{ $adjustment->status == 'Posted' ? 'fa-check' : 'fa-pencil' }} me-1"></i> {{ $adjustment->status }}
                    </span>
                    <span id="idBadge" class="badge bg-primary px-3 py-2 rounded-pill shadow-sm" style="font-size:12px;">
                        <i class="fa fa-tag me-1"></i> ID: {{ $adjustment->adj_id }}
                    </span>
                </div>
            ...
            <form action="{{ route('warehouse_stocks.update', $adjustment->id) }}" method="POST" id="stockUpdateForm" class="position-relative">
                @csrf
                @method('PUT')
                <input type="hidden" name="action" id="formAction" value="save">
                <div class="posted-watermark {{ $adjustment->status == 'Posted' ? 'show' : '' }}" id="postedWatermark">Posted</div>
                    ...
                    <div class="card-body pt-0">
                        <div class="row g-2">
                            ...
                            <div class="col-md-8">
                                ...
                                <input type="text" name="remarks" class="form-control input-sm" value="{{ $adjustment->remarks }}" placeholder="Optional notes for this update...">
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <div class="card shadow-sm">
                            ...
                            <table class="table table-bordered table-striped mb-0" id="itemsTable">
                                <thead class="bg-light">
                                    <tr>
                                        ...
                                        <th style="width:150px;">Qty Added/Adjusted</th>
                                ...
                                <tbody id="itemRows">
                                    @foreach($adjustment->items as $item)
                                        <tr>
                                            ...
                                            <td><input type="number" name="quantity[]" class="form-control input-sm quantity-input" value="{{ $item->qty }}" step="any"></td>
                    ...
                    <div class="card-footer bg-white py-3">
                        <div class="d-flex justify-content-end gap-2 text-end">
                            ...
                            {{-- Save Button --}}
                            <button type="button" id="saveBtn" class="btn btn-sm btn-warning rounded-pill px-4 shadow-sm" style="{{ $adjustment->status == 'Posted' ? 'display:none;' : '' }}">
                            ...
                            {{-- Post Button --}}
                            <button type="button" id="postBtn" class="btn btn-sm btn-primary rounded-pill px-4 shadow-sm" style="{{ $adjustment->status == 'Posted' ? 'display:none;' : '' }}">
                            ...
                            {{-- New Button --}}
                            <a href="{{ route('warehouse_stocks.create') }}" id="newBtn" class="btn btn-sm btn-info rounded-pill px-4 shadow-sm text-white">
...
$(document).ready(function() {
    ...
    var _adjustmentId = "{{ $adjustment->id }}";
    var _isPosted = "{{ $adjustment->status == 'Posted' ? '1' : '0' }}";
    ...
    $('#itemRows tr').each(function() {
        var $row = $(this);
        initProductSelect($row);
        var pid = $row.find('.item-id-input').val();
        if (pid) fetchCurrentStock($row, pid);
    });
    updateItemCount();
    ...
    function doSave(act) {
        ...
        $.ajax({
            url: "{{ route('warehouse_stocks.update', $adjustment->id) }}", 
            type: 'POST', 
            data: $form.serialize(),
            ...
            success: function(res) {
                if (res.success) {
                    ...
                    if(res.status === 'Posted') {
                        $('#statusBadge').removeClass('bg-info text-white').addClass('bg-success text-white').html('<i class="fa fa-check me-1"></i> Posted');
                        ...
                        $('#statusBadge').removeClass('bg-info text-white').addClass('bg-info text-white').html('<i class="fa fa-pencil me-1"></i> Unposted');
                        $('#editBtn, #newBtn').show();
                        $('#saveBtn, #postBtn').hide();
                        $('#stockUpdateForm').addClass('form-locked');
                        showToast('🔒 Draft Updated — Ctrl+E to Edit', 'success');
    ...
    $('#previewPrintBtn').on('click', function() {
        window.open("/warehouse_stocks/" + _adjustmentId + "/print", "_blank");
    });
